...
class Thermostat:
  ...
  def __init__(self, ip, 
    ...
    logging.basicConfig(format='%(asctime)s %(levelname)s:%(message)s', filename='./thermostat.log',level=logging.DEBUG)
  ...
  def read(self):
    ...
    logging.debug(' indoorTemp [%d] setpointDay [%d] setpointNight [%d] mode [%d] [%s] status [%d] [%s]' % (  self.indoorTemp, self.setpointDayValue, self.setpointNightValue, self.modeValue, Thermostat.mode[self.modeValue], self.state, Thermostat.status[self.state]) )    
  ...
  def setVariable (self, variable, value):
    if  variable< 52:
        ...
        elapsed = (time.time() - start) * 1000
        time.sleep(1)
        logging.debug(' setVariable [%i] = %i  [%d ms]' % ( int(variable), int(value), elapsed ) )
    else:
  ...
  def setMode(self, mode=0):
      if mode in Thermostat.mode:
        ...
        logging.debug(' Send mode %i[%s]  to %s ' % ( int(mode), Thermostat.mode[self.modeValue], self.ip) )
      else:
        logging.debug(' Send mode %i  to %s . Error: invalid mode' % ( int(mode), self.ip) )
  ...
  def update(self):
    ...
    logging.debug(' Send force-update scenario %i  to %s ' % ( int(self.thermostatScenarioId), self.ip) )
